...
serve(async (req) => {
  ...
  try {
    ...
    const deskPostureExtra = analysis_type === "desk_posture"
      ? ` CRITICAL FOR DESK POSTURE: 1) The webcam image is MIRRORED — the user's LEFT appears on screen-RIGHT. Correct all left/right references to the user's actual anatomy. 2) Compute actual angle measurements for head tilt (from ear heights), shoulder height (from shoulder landmarks), trunk lean (sternum-to-pelvis vs vertical), and clavicle line. 3) If ANY angle >= 2°, do NOT report neutral — report the asymmetry with its measured angle. 4) Separate findings into observed_findings (what you can see) and possible_contributing_factors (what you infer). 5) Use plain language — no clinical jargon in the main report. 6) Only include body_map regions with high confidence (2-4 per category max). 7) Do NOT over-specify muscles unless confidence is high — prefer broader descriptions. 8) SHOULDER ASYMMETRY IS CRITICAL: Compare left vs right shoulder vertical position very carefully. Even a small visible difference (3-5°) MUST be reported as shoulder height asymmetry — NEVER skip this. If one shoulder appears even slightly higher, report it, identify which side is elevated, and explain the likely muscle imbalance (e.g. upper trap/levator overactivity on that side, lower trap/serratus weakness on the opposite side). 9) You MUST always populate muscle_imbalances with at least one entry describing overactive_tight muscles, underactive_weak muscles, AND possible_injuries/risk areas for any detected asymmetry. 10) You MUST always populate symptom_correlation with at least one entry mapping the detected pattern to likely symptom areas. 11) Even for mild findings, describe what muscles may be weak/underactive and what injury risks exist.`
      : "";
